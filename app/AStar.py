# from https://github.com/ericdand/battlesnake-python/blob/5b38d75287272e6fe47e07029e2b3c6e8682400b/app/AStar.py
# used 2 years ago
...
def neighbours(node, grid, score, tail, ignore_list):
    ...
    result = filter(lambda p: (
        grid[p[0]][p[1]] not in ignore_list) or (p in subtail), result)
...
def a_star(start, goal, grid, tail):
    ...
    g_score = [[10000 for x in xrange(len(grid[y]))]
               for y in xrange(len(grid))]
    g_score[start[0]][start[1]] = 0

    f_score = [[10000 for x in xrange(len(grid[y]))]
               for y in xrange(len(grid))]
    ...
    while(len(open_set) > 0):
        ...
        for neighbour in neighbours(current, grid, g_score[current[0]][current[1]], tail, [SNAKE, WALL, SAFETY]):
            if neighbour in closed_set:
                continue
            tentative_g_score = g_score[current[0]][
                current[1]] + dist(current, neighbour)
            ...
            f_score[neighbour[0]][neighbour[1]
                                  ] = tentative_g_score + dist(neighbour, goal)

    return None

# print a_star((3,1),(0,1),[[0,2,0,0],[0,2,0,0],[1,2,0,0],[0,2,0,0]], [[3,1],[2,1],[1,1],[0,1]])
# print
# a_star((0,4),(3,4),[[0,2,0,0,2,0],[0,2,0,1,2,2],[0,2,2,2,2,2],[0,0,1,0,0,0]],[[0,4],[1,4],[1,5],[2,5],[2,4],[2,3],[2,2],[2,1],[1,1],[1,0]])
